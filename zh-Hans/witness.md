...
按照目前每个块`3SEER`的奖励，目前的块间隔为`3秒`。即每年的出块奖励支出约为 `365`（日）x `24`（时）x `60`（分）x `60`（秒）/ `3`（块间隔）x `3`（出块奖励）= `3153.6万` SEER，每个主力见证人在未缺块的情况下，每年可以获得`约150万`SEER的出块奖励。
...
主力见证人`20年`的出块奖励 = `31536000` x `20` = `630720000` ，`15年`约等于`5400天`。
